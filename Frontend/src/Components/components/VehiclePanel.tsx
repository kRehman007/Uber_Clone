import { FaRegUser } from "react-icons/fa";
import { IoIosArrowDown } from "react-icons/io";
import { FareResponse } from "../Utils/interfaces";

import { Skeleton } from "../ui/skeleton";
import { Table, TableBody, TableCell, TableRow } from "../ui/table";

interface VehiclePanelProps {
  setSelectVehicleType: (value: keyof FareResponse) => void;
  fare: FareResponse | null;
  setVehiclePanelOpen: (value: boolean) => void;
  setConfirmRidePanelOpen: (value: boolean) => void;
}
const VehiclePanel: React.FC<VehiclePanelProps> = ({
  setVehiclePanelOpen,
  setConfirmRidePanelOpen,
  setSelectVehicleType,
  fare,
}) => {
  return (
    <div>
      <IoIosArrowDown
        className=" text-2xl w-full  floar-right"
        onClick={() => setVehiclePanelOpen(false)}
      />
      <h2 className="text-2xl font-semibold mb-5 font-sans">
        Choose a Vehilce
      </h2>
      {!fare ? (
        <>
          {[1, 2, 3].map((_, index) => (
            <div
              key={index}
              className="flex my-3 rounded-lg items-center space-x-8 border p-3"
            >
              <Skeleton className="h-16 w-16 rounded-full" />
              <div className="space-y-2">
                <Skeleton className="h-3 w-[200px]" />
                <Skeleton className="h-3 w-[150px]" />
              </div>
            </div>
          ))}
        </>
      ) : (
        <>
          <Table
            className=" border w-full active:border-2 active:border-black "
            onClick={() => {
              setConfirmRidePanelOpen(true);
              setVehiclePanelOpen(false);
              setSelectVehicleType("car");
            }}
          >
            <TableBody>
              <TableRow>
                <TableCell className="font-medium h-24 w-24 ">
                  <img
                    className="w-full h-full object-cover overflow-hidden"
                    src="data:image/jpeg;base64,"
                  />
                </TableCell>
                <TableCell className="pl-7">
                  <div className="flex gap-7 items-center">
                    <h2 className="text-medium font-semibold">UberGo </h2>
                    <span className="flex gap-1 items-center">
                      <FaRegUser className="text-medium" />
                      <span className="text-lg font-medium">4</span>
                    </span>
                  </div>
                  <h4 className="text-sm font-semibold">2 mins away</h4>
                  <p className="text-sm text-gray-500 -mt-0.5 ">
                    Afforrdable car ride
                  </p>
                </TableCell>
                <TableCell className="text-right text-lg font-bold">
                  {fare?.car}
                  <small className="text-xs font-normal ml-1 mr-1">Rs</small>
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
          <Table
            className=" border w-full my-4 active:border-2 active:border-black "
            onClick={() => {
              setConfirmRidePanelOpen(true);
              setVehiclePanelOpen(false);
              setSelectVehicleType("motorcycle");
            }}
          >
            <TableBody>
              <TableRow>
                <TableCell className="font-medium h-24 w-24 ">
                  <img
                    className="w-full h-full object-cover overflow-hidden"
                    src="data:image/jpeg;base64,"
                  />
                </TableCell>
                <TableCell className="pl-7">
                  <div className="flex gap-7 items-center">
                    <h2 className="text-medium font-semibold">UberGo </h2>
                    <span className="flex gap-1 items-center">
                      <FaRegUser className="text-medium" />
                      <span className="text-lg font-medium">1</span>
                    </span>
                  </div>
                  <h4 className="text-sm font-semibold">2 mins away</h4>
                  <p className="text-sm text-gray-500 -mt-0.5 ">
                    Afforrdable motorcycle ride
                  </p>
                </TableCell>
                <TableCell className="text-right text-lg font-bold">
                  {fare?.motorcycle}
                  <small className="text-xs font-normal ml-1 mr-1">Rs</small>
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
          <Table
            className=" border w-full active:border-2 active:border-black "
            onClick={() => {
              setConfirmRidePanelOpen(true);
              setVehiclePanelOpen(false);
              setSelectVehicleType("auto");
            }}
          >
            <TableBody>
              <TableRow>
                <TableCell className="font-medium h-24 w-24 ">
                  <img
                    className="w-full h-full object-cover overflow-hidden mix-blend-darken"
                    src="data:image/jpeg;base64,"
                  />
                </TableCell>
                <TableCell className="pl-7">
                  <div className="flex gap-7 items-center ">
                    <h2 className="text-medium font-semibold">UberGo </h2>
                    <span className="flex gap-1 items-center">
                      <FaRegUser className="text-medium" />
                      <span className="text-lg font-medium">3</span>
                    </span>
                  </div>
                  <h4 className="text-sm font-semibold">2 mins away</h4>
                  <p className="text-sm text-gray-500 -mt-0.5 ">
                    Afforrdable auto ride
                  </p>
                </TableCell>
                <TableCell className="text-right text-lg font-bold">
                  {fare?.auto}
                  <small className="text-xs font-normal ml-1 mr-1">Rs</small>
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </>
      )}
    </div>
  );
};

export default VehiclePanel;
